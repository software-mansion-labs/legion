...
- Add `Legion.RateLimiter` for applying rolling agent and token limits, with a Postgres adapter that extends `Legion.Store.Postgres`; the Store migration creates the rate-limit metadata column and index. Configure `:rate_limit` with a limiter, policy, and key per `start_link/2` call or globally. The key defines the shared cohort whose matching agents consume the same rolling-window limits. Legion admits every turn through the limiter before appending the message or calling the LLM; a refused turn returns `{:cancel, {:rate_limited, violations}}` and emits `[:legion, :rate_limit, :exceeded]`. Sub-agents inherit their parent's resolved rate-limit configuration; resumed and recovered runs are not re-admitted
...
- Add `Legion.Sandbox.Popcorn` - evaluates agent code in the user's browser via [popcorn](https://github.com/software-mansion/popcorn/) (AtomVM on WebAssembly), with tool calls relayed to the host application. The consuming app runs `{Legion, []}` in its supervision tree (which starts the `Legion.Sandbox.Popcorn.Host` registry), bridges a browser session to it (e.g. a Phoenix Channel), and serves the browser runtime from `priv/popcorn`
