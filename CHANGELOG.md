...
- Add `Legion.RateLimiter` for applying rolling agent and token limits, with a Postgres adapter that extends `Legion.Store.Postgres`; the Store migration creates the rate-limit metadata column and index. Configure `:rate_limit` with a limiter, policy, and key per agent or globally and Legion admits every turn through the limiter before appending the message or calling the LLM; a refused turn returns `{:cancel, {:rate_limited, violations}}` and emits `[:legion, :rate_limit, :exceeded]`. Sub-agents inherit their parent's resolved rate-limit configuration; resumed and recovered runs are not re-admitted
