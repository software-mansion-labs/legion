...
- Add configurable per-request LLM usage persistence, enabled by default and disabled globally with `config :legion, :track_usage, false`. `Legion.Store.Postgres` stores usage as `jsonb[]`.
