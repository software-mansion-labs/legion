...
- Add `Legion.RateLimiter` for applying rolling agent and token limits, with a Postgres adapter that extends `Legion.Store.Postgres`; the Store migration creates the rate-limit metadata column and index. Configure `:rate_limit` with a limiter and a list of `Legion.RateLimiter.Rule`s, each pairing an identity - the cohort whose agents share the rolling-window limits, e.g. per IP or per email - with a `Legion.RateLimiter.Policy`; the limiter and a default policy can be set globally. Legion admits every turn through all rules, in order and atomically, before appending the message or calling the LLM; the first refusing rule returns `{:cancel, {:rate_limited, violations}}` and emits `[:legion, :rate_limit, :exceeded]`. Sub-agents inherit their parent's resolved rate-limit configuration; resumed and recovered runs are not re-admitted
